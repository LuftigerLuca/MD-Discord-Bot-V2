package eu.luftiger.mdbot.discord.model.modal

import eu.luftiger.mdbot.discord.DiscordBot
import eu.luftiger.mdbot.discord.util.isDate
...
class SickNoteModal(private val discordBot: DiscordBot) : ListenerAdapter() {

    fun reply(event: GenericComponentInteractionCreateEvent) {
        if (!discordBot.guildsProvider.hasPermission(event.guild!!.id, event.user.id, "sicknote_create")) {
    ...
    override fun onModalInteraction(event: ModalInteractionEvent) {
        ...
        val botGuild = discordBot.guildsProvider.getGuild(guild.id)
        if (botGuild == null) {
